...
| **Time bias** | DTE bucket, hold days, entry weekday/session | `pcs_time_bias_grid.py` covers multi-hyp DTE/profit-target/DTE-stop + entry-weekday/cost slices; completed-bar filters and chronological selection/holdout are built; append-safe 30m + daily-feature archives expanded the completed-30m open/midday/late PCS/CCS/IC lab from 21 to all 60 raw dates without same-date features, but its locked 8-symbol rerun still rejected (1/24 train passes, 0/24 complete train+holdout passes) |
...
| `double_diagonal_spread` | double_diagonal_sim | Long-debit neutral time spread | Neutral/high IV; stand aside outside configured regime | Front/back DTE, short delta, inward back wing, IV multipliers, target/stop | Structural debit before closing friction; operating max 1 lot | American-intrinsic-floor BS scaffold + signed four-leg cost accounting + one-lot operating cap (`theoretical_max_lots` separately labels capacity) + reproducible 60/40 reject-unless lab; 0/8 pass both costs, no registration/L1 |
...
Cost realism now includes `scripts/defined_risk_fixed_cost_stress.py`: one uniform fixed-dollar option-price half-spread per leg, applied adversely at entry and exit for PCS/CCS/IC/iron butterfly/calendar/diagonal/butterfly/debit-vertical proxies. The double-diagonal scaffold applies the same signed adverse-leg principle directly, sizes capital to the larger of structural debit and observed stressed path loss, enforces operating `max_lots=1`, and retains generic capacity only as `theoretical_max_lots`; none of these proxy axes is observed quote evidence or a hard live fill bound. `scripts/option_quote_observations.py` adds a normalized bid/ask archive boundary; capture now fetches every available expiration by default, appends atomically without duplicating identical rows, and reports a fail-closed three-market-date density gate. Synthetic fixtures are calibration-ineligible. `scripts/observed_quote_coverage.py` adds an exact non-future PCS/CCS/IC strategy-leg/expiry/strike/time join and rejects incomplete entry/exit coverage. `ArchivedContractGridProvider` turns normalized observed rows into market-date-specific actual expiry/right/strike grids with request coverage counters; missing dates fail closed. Synthetic mode remains explicit for discovery. The all-expiration TSLL archive still covers only 2026-07-11, so provider-backed historical simulation and observed-cost calibration remain blocked until at least three distinct market dates (`docs/OPTION_QUOTE_DATA_BOUNDARY.md`).
...
### 2026-07-13 — Double-diagonal scaffold + chronological rejection

Added `double_diagonal_spread` as a structure-pure 14/60-DTE four-leg Black-Scholes daily-bar scaffold with same/inward protective back legs, American intrinsic floors, signed percentage/fixed-dollar entry and exit costs, exact ledger/no-close-bar-reentry checks, capital sizing to the larger of structural debit or observed stressed path loss, and an enforced one-lot operating cap with generic capacity retained only as `theoretical_max_lots`. `scripts/double_diagonal_chronological_lab.py` reproducibly evaluates BAC/F/SOFI/PLTR/TSLL/SMCI/AMD/AAPL on 60/40 train/untouched holdout plus 63-bar holdout windows. The predeclared run completed 8/8 with 0/8 all-axis passes: F fixed-cost holdout was only +$7.33 but failed $84.82 DD and its train lost $179.35; SOFI fixed-cost holdout was +$214.34 but failed $317.35 DD and its train lost $291.90; TSLL fixed-cost train/holdout were positive (+$92.34/+239.88) but DD failed at $119.15/$129.22, and 5% slip lost $538.36/$372.81. No hypothesis, L1 seat, paper intent, shadow, arm, or live action was created. This is L0 proxy rejection; observed option surfaces and assignment/exercise calibration remain prerequisites.
