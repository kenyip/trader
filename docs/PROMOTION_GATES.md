...
Code: `trader_platform/promotion_gates.py`  
...
| risk_envelope | Fits `trader_platform/risk_limits.yaml` |
...
```bash
.venv/bin/python -m trader_platform.hypothesis_cli gate hyp_short_premium_tsla --target live
.venv/bin/python -m trader_platform.hypothesis_cli status hyp_pmcc_income testing
.venv/bin/python -m trader_platform.hypothesis_cli status hyp_x live --evidence path/to/report.md
```
